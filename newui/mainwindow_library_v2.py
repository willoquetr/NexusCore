...
class GameCard(QFrame):
    ...
    def __init__(self, title, launcher, exe_path=None, parent_view=None):
        ...
        self.launcher_badge = QLabel(str(launcher).upper())
...
class GamesView(QWidget):
    ...
    def set_launcher_filter(self, launcher):
        ...
        # Ensure single active state
        for name, btn in getattr(self, 'filter_buttons', {}).items():
            if name != launcher:
    ...
    def refresh_games(self):
        ...
        # Apply launcher filter
        filtered = all_items
        try:
            flt = getattr(self, "active_launcher", "TOUS")
            if flt and flt != "TOUS":
                filtered = [it for it in all_items if it.get("launcher", "").lower().startswith(flt.lower().replace(".NET","").replace(".","")) or it.get("launcher", "").lower() == flt.lower()]
        except:
            filtered = all_items

        self.parent_win.session_mgr.update_games_list(filtered)
